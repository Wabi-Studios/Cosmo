    ...
    { // swiftlint:disable:this function_body_length
      ...
      { fileItem in
        ...
        for (lineNumber, line) in string.split(separator: "\n").lazy.enumerated()
        {
          ...
          {
            // We've got a match
            let matches = noSpaceLine.ranges(of: textToCompare).map
            { range in
...
extension String
{
  ...
  {
    guard !substring.isEmpty, contains(substring) else { return [] }
    var appearances: [Range<String.Index>] = []
    for (index, character) in enumerated() where character == substring.first
    {
