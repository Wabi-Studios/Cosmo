...
@objc(WorkspaceDocument) final class WorkspaceDocument: NSDocument, ObservableObject, NSToolbarDelegate
{
  ...
  override func makeWindowControllers()
  {
    ...
    let windowController = CosmoWindowController(
      window: window,
      workspace: self
    )

    windowController.shouldCascadeWindows = true
    windowController.window?.setFrameAutosaveName(fileURL?.absoluteString ?? "Untitled")
...
//        )
    workspaceFileManager = .init(
      folderUrl: url,
      ignoredFilesAndFolders: ignoredFilesAndDirectory
    )
    searchState = .init(self)
    quickOpenViewModel = .init(fileURL: url)
    commandsPaletteState = .init()
  }
  ...
  override func write(to _: URL, ofType _: String) throws {}
