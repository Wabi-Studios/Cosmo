...
struct TabBarView: View
{
    ...
    DragGesture(minimumDistance: 2, coordinateSpace: .global)
      ...
      { _ in
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.40)
        {
          ...
          {
            tabgroup.tabs = .init(openedTabs.compactMap
            { id in
      ...
      { geometryProxy in
        ScrollView(.horizontal, showsIndicators: false)
        {
          ...
          { scrollReader in
            ...
            {
              ForEach(Array(openedTabs.enumerated()), id: \.element)
              { index, id in
            ...
            .onChange(of: tabgroup.tabs)
            { [tabs = tabgroup.tabs] newValue in
            ...
            .onChange(of: geometryProxy.size.width)
            { _ in
              updateExpectedTabWidth(proxy: geometryProxy)
              withAnimation
              {
  ...
  private struct TabBarItemOnDropDelegate: DropDelegate
  {
    ...
    )
    {
      ...
      _onDragTabId = onDragTabId
      _onDragLastLocation = onDragLastLocation
      _isOnDragOverTabs = isOnDragOverTabs
      _tabWidth = tabWidth
    }
