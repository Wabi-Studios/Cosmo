...
struct ToolbarBranchPicker: View
{
  ...
  )
  {
    ...
    {
      gitClient = GitClient(directoryURL: folderURL, shellClient: shellClient)
    }
    _currentBranch = State(initialValue: try? gitClient?.getCurrentBranchName())
  }
