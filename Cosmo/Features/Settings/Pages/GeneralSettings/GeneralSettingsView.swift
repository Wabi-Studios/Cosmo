...
struct GeneralSettingsView: View
{
  ...
  @State private var openInCosmo: Bool = true
  ...
  init()
  {
    ...
    openInCosmo = defaults.bool(forKey: "enableOpenInCE")
  }
...
private extension GeneralSettingsView
{
  ...
  var findNavigatorDetail: some View
  {
    ...
    {
      ForEach(SettingsData.NavigatorDetail.allCases, id: \.self)
      { tag in
  ...
  {
    LabeledContent("'cosmo' Shell Command")
    {
  ...
  func installShellCommand()
  {
    do
    {
      let url = Bundle.main.url(forResource: "cosmo", withExtension: nil, subdirectory: "Resources")
      let destination = "/usr/local/bin/cosmo"
      ...
      NSWorkspace.shared.requestAuthorization(to: .createSymbolicLink)
      { auth, error in
        guard let auth, error == nil
        else
        {
  ...
  var openInCosmoToggle: some View
  {
    Toggle("Show “Open With Cosmo” option in Finder", isOn: $openInCosmo)
      .onChange(of: openInCosmo)
      { newValue in
        guard let defaults = UserDefaults(
          suiteName: "foundation.wabi.cosmo.shared"
        )
