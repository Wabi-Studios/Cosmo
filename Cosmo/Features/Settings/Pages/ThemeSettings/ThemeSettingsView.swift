      ...
      Section
      {
        VStack(spacing: 0)
        {
          if settings.matchAppearance
          {
            ...
            {
              ForEach(ThemeSettingsAppearances.allCases, id: \.self)
              { tab in
                Text(tab.rawValue)
                  .tag(tab)
              }
          ...
          VStack(spacing: 0)
          {
            ForEach(selectedAppearance == .dark ? themeModel.darkThemes : themeModel.lightThemes)
            { theme in
              Divider()
            ...
            ForEach(selectedAppearance == .dark ? themeModel.lightThemes : themeModel.darkThemes)
            { theme in
              Divider()
              ThemeSettingsThemeRow(
                theme: $themeModel.themes[themeModel.themes.firstIndex(of: theme)!],
...
private extension ThemeSettingsView
{
  ...
  private var changeThemeOnSystemAppearance: some View
  {
    ...
    .onChange(of: settings.matchAppearance)
    { value in
      if value
      {
        if colorScheme == .dark
