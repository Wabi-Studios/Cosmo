...
struct GitHubTokenConfiguration: GitRouterConfiguration
{
  ...
  let errorDomain: String? = "foundation.wabi.models.accounts.github"
...
struct GitHubOAuthConfiguration: GitRouterConfiguration
{
  ...
  let errorDomain = "foundation.wabi.models.accounts.github"
        ...
        if let response = response as? HTTPURLResponse
        {
          ...
          else
          {
            if let data, let string = String(data: data, encoding: .utf8)
            {
              ...
              if let accessToken
              {
                let config = GitHubTokenConfiguration(accessToken, url: apiEndpoint ?? "")
                completion(config)
              }
            }
          }
