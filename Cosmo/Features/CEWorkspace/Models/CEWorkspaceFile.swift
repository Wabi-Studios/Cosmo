...
final class CEWorkspaceFile: Codable, Comparable, Hashable, Identifiable, TabBarItemRepresentable
{
  ...
  var doesExist: Bool { CEWorkspaceFile.fileManger.fileExists(atPath: url.path) }
  ...
  /// If it is a `.cosmo` folder this will return `"folder.fill.badge.gearshape"`.
  ...
  func addFolder(folderName: String)
  {
    ...
    var folderUrl = (isFolder ?
      url.appendingPathComponent(folderName) :
      url.deletingLastPathComponent().appendingPathComponent(folderName))
  ...
  func addFile(fileName: String)
  {
    ...
    var fileExtensions = ["": 0]

    for child in (isFolder ?
      flattenedSiblings(withHeight: 2, ignoringFolders: true) :
  ...
  func delete()
  {
    ...
    let message: String
    if isFolder || (children?.isEmpty ?? false)
    { // if its a file or an empty folder, call it by its name
      message = String(describing: fileName)
    }
    else
    {
      message = "the \((children?.count ?? 0) + 1) selected items"
    }
    ...
    if deleteConfirmation.runModal() == .alertFirstButtonReturn
    { // "Delete" button
      if CEWorkspaceFile.fileManger.fileExists(atPath: url.path)
      {
        do
        {
          try CEWorkspaceFile.fileManger.removeItem(at: url)
        }
        catch
        {
          fatalError(error.localizedDescription)
  ...
  func duplicate()
  {
    ...
    if CEWorkspaceFile.fileManger.fileExists(atPath: url.path)
    {
      do
      {
        try CEWorkspaceFile.fileManger.copyItem(at: url, to: fileUrl)
      }
      catch
      {
        fatalError(error.localizedDescription)
  ...
  func move(to newLocation: URL)
  {
    ...
    do
    {
      try CEWorkspaceFile.fileManger.moveItem(at: url, to: newLocation)
    }
