...
/// This class is used to load the files of the machine into a Cosmo workspace.
final class CEWorkspaceFileManager
{
  ...
  init(folderUrl: URL, ignoredFilesAndFolders: [String])
  {
    ...
    workspaceItem = CEWorkspaceFile(url: folderUrl, children: [])
  ...
  private func setup()
  {
    ...
    do
    {
      workspaceFiles = try loadFiles(fromUrl: folderUrl)
    }
    catch
    {
    ...
    // workspace fileItem
    let workspaceFile = CEWorkspaceFile(url: folderUrl, children: workspaceFiles)
  ...
  {
    guard let item = flattenedFileItems[id]
    else
    {
  ...
  func reloadFromWatcher(sourceFileItem: CEWorkspaceFile)
  {
    ...
    { // this runs when a file change is detected but is already running
  ...
  func rebuildFiles(fromItem fileItem: CEWorkspaceFile) throws -> Bool
  {
    ...
    for newContent in directoryContentsUrls
    {
      ...
      guard !(fileItem.children?.map(\.url).contains(newContent) ?? false) else { continue }
