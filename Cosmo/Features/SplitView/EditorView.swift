...
struct EditorView: View
{
  ...
  struct SubEditorView: View
  {
    ...
    {
      ForEach(Array(data.tabgroups.enumerated()), id: \.offset)
      { index, item in
        EditorView(tabgroup: item, focus: $focus)
          .transformEnvironment(\.isAtEdge)
          { belowToolbar in
            calcIsAtEdge(current: &belowToolbar, index: index)
          }
          .environment(\.splitEditor)
          { [weak data] edge, newTabGroup in
...
{
  var isAtEdge: BelowToolbarEnvironmentKey.Value
  {
