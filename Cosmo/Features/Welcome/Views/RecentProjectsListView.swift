...
struct RecentProjectsListView: View
{
  ...
  init(openDocument: @escaping (URL?, @escaping () -> Void) -> Void, dismissWindow: @escaping () -> Void)
  {
    ...
    _selection = .init(wrappedValue: Set(Array(projects.prefix(1))))
  }
  ...
  var body: some View
  {
    ...
    {
      List(recentProjects, id: \.self, selection: $selection)
      { project in
        RecentProjectItem(projectPath: project)
      }
      .listStyle(.sidebar)
      .contextMenu(forSelectionType: URL.self)
      { items in
        switch items.count
        {
          case 0:
          ...
          default:
            ...
            Button("Remove from Recents")
            {
              ...
              oldItems.filter { !items.contains($0) }.reversed().forEach
              { url in
      ...
      .onDeleteCommand
      {
        let oldItems = NSDocumentController.shared.recentDocumentURLs
        NSDocumentController.shared.clearRecentDocuments(nil)
        oldItems.filter { !selection.contains($0) }.reversed().forEach
        { url in
          NSDocumentController.shared.noteNewRecentDocumentURL(url)
        }
