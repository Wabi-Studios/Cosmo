...
struct TerminalEmulatorView: NSViewRepresentable
{
  ...
  {
    self.url = url
    _terminal = State(initialValue: TerminalEmulatorView.lastTerminal[url.path] ?? .init(frame: .zero))
  }
  ...
  private var colors: [SwiftTerm.Color]
  {
    ...
    {
      return themeModel.themes[index].terminal.ansiColors.map
      { color in
  ...
  func updateNSView(_ view: LocalProcessTerminalView, context _: Context)
  {
    if view.font != font
    { // Fixes Memory leak
      view.font = font
